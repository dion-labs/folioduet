...
| START-02 | Reload while an existing guest/Google session is restoring on a slow network. | Existing account is restored; no guest is minted over Google; library is neither erased nor briefly exposed from a different account. |
...
| LIB-02 | Reload/reopen after imports and progress changes. | Titles, active document, progress and processed-content status persist; no repeated unwanted onboarding or duplicate demo. |
...
| READ-06 | Reload at a late page; reopen cached pack; change font/viewport; use a fixture with old/missing stream index. | Resume does not jump to page 1 or become permanently clamped; saved page/stream fallback follows valid anchors; stale packs are rejected when appropriate. |
